...
import { getCityBySlug, getCities, getAirports, toNumber } from "@/lib/get-site-data";
import { getCars } from "@/lib/data";
import { CarCard } from "@/components/car-card";
import { MapPin, Plane, Building2, Clock, Shield, CreditCard, Phone, ChevronRight, Star, Check, ArrowRight, BadgeCheck, Users, Zap } from "lucide-react";
...
export async function generateCityMetadata(citySlug: string): Promise<Metadata> {
    ...
    return {
        ...
        openGraph: {
            title: city.metaTitle,
            description: city.metaDescription,
            images: city.heroImage ? [`${siteConfig.url.baseUrl}${city.heroImage}`] : [],
        },
        alternates: {
            canonical: `${siteConfig.url.baseUrl}/location-voiture-${city.slug}`,
        },
    };
}

export default async function CityPageTemplate({ citySlug }: CityPageTemplateProps) {
    const city = await getCityBySlug(citySlug);
    if (!city) notFound();

    const [cars, allCities, allAirports] = await Promise.all([
        getCars(),
        getCities(),
        getAirports(),
    ]);
    const offset = citySlug.split('').reduce((acc, c) => acc + c.charCodeAt(0), 0) % Math.max(cars.length - 4, 1);
    ...
    const fromPrice = toNumber(city.fromPrice);

    const jsonLd = {
        "@context": "https://schema.org",
        "@type": "LocalBusiness",
        name: `${siteConfig.brand.name} - ${city.name}`,
        description: city.description,
        image: city.heroImage ? `${siteConfig.url.baseUrl}${city.heroImage}` : undefined,
        address: {
            "@type": "PostalAddress",
        ...
        priceRange: `${fromPrice} DT - 1521 DT`,
    };
    ...
    return (
        <>
            ...
            <BreadcrumbSchema items={[
                ...
                { name: `Location voiture ${city.name}`, url: `${siteConfig.url.baseUrl}/location-voiture-${city.slug}` },
            ]} />
            ...
            <section className="bg-[#00256f]">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 py-5 sm:py-6">
                    <nav className="flex items-center gap-1.5 text-xs text-blue-300/70 mb-3">
                        ...
                        <span className="text-white font-medium">Location voiture {city.name}</span>
                    </nav>
                    <div className="flex items-center justify-between gap-4">
                        <div>
                            <h1 className="text-xl sm:text-2xl md:text-3xl font-black text-white leading-tight">
                                Location Voiture <span className="text-on-primary-container">{city.name}</span>
                            </h1>
                            <p className="text-xs sm:text-sm text-blue-200/70 mt-1">
                                Kilométrage illimité &bull; Assurance incluse &bull; Sans carte bancaire &bull; Livraison gratuite
                            </p>
                        </div>
                    ...
                    <div className="flex items-end justify-between mb-5">
                        <div>
                            <h2 className="text-xl sm:text-2xl font-bold text-gray-900">
                                Voitures disponibles {city.nameWithArticle}
                            </h2>
                            <p className="text-sm text-gray-500 mt-1">Livraison gratuite à {city.name} et environs</p>
                        </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div className="text-center sm:text-left">
                            <h2 className="text-lg sm:text-xl font-bold text-white">
                                Réservez votre voiture {city.nameWithArticle} dès maintenant
                            </h2>
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            <div>
                                <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-3">
                                    Location de voiture {city.nameWithArticle}
                                </h2>
                            ...
                            {city.airport && (
                                <div className="bg-blue-50 rounded-2xl p-5 sm:p-6 border border-blue-100">
                                    <h3 className="text-base font-bold text-gray-900 mb-2 flex items-center gap-2">
                                        ...
                                        Service aéroport — {city.airport}
                                    </h3>
                                    <p className="text-gray-600 text-sm mb-3">
                                        Votre voiture vous attend à la sortie de l&apos;aéroport. Service disponible 24h/24, même pour les vols tardifs.
                                    </p>
                    {(() => {
                        const linkedAirport = allAirports.find(a => a.city === city.name);
                        return linkedAirport ? (
                                            <Link
                                                href={`/aeroport-${linkedAirport.slug}`}
                            ...
                            <div className="bg-gray-50 rounded-2xl p-5 sm:p-6">
                                <h3 className="text-base font-bold text-gray-900 mb-3 flex items-center gap-2">
                                    ...
                                    Livraison hôtels {city.nameWithArticle}
                                </h3>
                            ...
                            <div className="sticky top-20 space-y-5">
                                ...
                                <div className="bg-white rounded-2xl p-5 border-2 border-accent/20 shadow-lg shadow-accent/5">
                                    <div className="text-center mb-4">
                                        ...
                                        <div className="text-xs text-gray-500 mb-1">Location {city.nameWithArticle} dès</div>
                                    ...
                                    <ul className="space-y-2.5 mb-5">
                                        {[
                                            ...
                                            "Livraison gratuite aéroport & hôtel",
                                            "Sans carte bancaire",
                                ...
                                <div className="bg-gray-50 rounded-2xl p-5">
                                    ...
                                    <p className="text-sm text-gray-600 leading-relaxed italic">
                                        &ldquo;Service impeccable à {city.name}. Voiture livrée à l&apos;heure, propre et en excellent état. Je recommande sans hésiter.&rdquo;
                                    </p>
                ...
                <section className="py-8 sm:py-10 bg-gray-50">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6">
                        <h2 className="text-xl font-bold text-gray-900 mb-2">
                            À découvrir {city.nameWithArticle}
                        </h2>
                        <p className="text-gray-500 text-sm mb-5">
                            Louez une voiture et explorez les incontournables de {city.name}
                        </p>
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                            {city.attractions.map((attraction, i) => (
                                <div key={i} className="bg-white rounded-xl p-4 text-center border border-gray-100 hover:border-accent/30 hover:shadow-sm transition-all">
                                    <div className="font-semibold text-gray-900 text-sm">{attraction}</div>
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            <section className="py-8 sm:py-10 bg-white">
                <div className="max-w-7xl mx-auto px-4 sm:px-6">
                    <h2 className="text-xl font-bold text-gray-900 mb-5">
                        Autres destinations en Tunisie
                    </h2>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                        {allCities.filter(c => c.slug !== city.slug).map((c) => (
                            <Link
                                ...
                                className="bg-gray-50 rounded-xl p-3 border border-gray-100 hover:border-accent hover:shadow-md transition-all text-center group"
                            >
                                <div className="font-bold text-gray-900 text-sm group-hover:text-accent transition-colors">
                                    {c.name}
                                </div>
                                <div className="text-xs text-accent font-semibold"><CurrencyPrice amount={toNumber(c.fromPrice)} className="" /></div>
                            </Link>
                        ))}
                    </div>
                </div>
            ...
            <section className="bg-[#00256f]">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8">
                        {[
                            { icon: Clock, title: "Service 24h/24", desc: "Livraison jour et nuit à " + city.name },
